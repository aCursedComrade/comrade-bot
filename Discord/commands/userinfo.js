...
export async function handler(interaction) {
    ...
    await interaction.guild.members
        .fetch(interaction?.options.getUser('user', false) || interaction.user.id)
        .then(async (member) => {
            const roles =
                member.roles.cache
                    .map((role) => role)
                    .toString()
                    .replace(/,/g, ' ') || '(User has no roles)';
            const embed = new EmbedBuilder()
                ...
                .setFields([
                    {
                        name: 'Registered on:',
                        value: `<t:${Math.floor(member.user.createdAt.valueOf() / 1000)}:D>`,
                        inline: true,
                    },
                    {
                        name: 'Joined on:',
                        value: `<t:${Math.floor(member.joinedAt.valueOf() / 1000)}:D>`,
                        inline: true,
                    },
                    {
